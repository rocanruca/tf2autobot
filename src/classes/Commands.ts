...
export = class Commands {
        ...
        */
        ...
        log.debug(
            'isActive: ' +
                ...
                ', isBuying: ' +
                status.isBuying.toString() +
                ', isBanking: ' +
                status.isBanking.toString()
        );
    }
    ...
    private getItemAndAmount(steamID: SteamID, message: string): { match: Entry; amount: number } | null {
        ...
        if (match === null) {
            this.bot.sendMessage(
                ...
                • Some Taunt needs "The" such as "Taunt: The High Five!", while others do not.
    ...
    private craftWeapons(): string[] {
        ...
        craftWeapons.forEach(sku => {
            ...
            if (process.env.BANKING_WEAPONS === 'false') {
                if (amount > 0) {
                    items.push({
                        name: this.bot.schema.getName(SKU.fromString(sku), false),
                        amount: amount
